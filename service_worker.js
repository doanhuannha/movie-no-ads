...
const resourceUrl = 'http://dev-world.net/extensions/movie-no-ads1';
//const resourceUrl = 'http://localhost:7770/extensions/movie-no-ads1';
let movie_sites = null, ruleId = 5000, scriptId = 10;
const fetchJson = async (urls) => {
    for (let url of urls) {
        let json = await fetch(url, { cache: 'no-store' })
            .then(r => r.json())
            .then(r => { return r })
            .catch(() => { return null });
        if (json) return json;
...
cleanUp().then(() => {
    ...
    fetchJson([resourceUrl + '/configs.json', './configs.json']).then(configs => {
